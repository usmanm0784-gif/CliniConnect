...
async def update_doctor_data(updated_data, id):
...
async def get_slots(doctor_id):# add slot id
    slots = await slots_collection.find({"doctor_id": doctor_id}).to_list(length=None)
    return slots
...
async def get_doctor_phone_number(phone_number):
    doctor = await doctors_collection.find_one({"phone_number": phone_number})
    return doctor

async def get_doctor_by_id(id):
