...
from models.doctorupdatemodel import AvailabilitySlot, DoctorUpdate
...
# get all doctors
@router.get("/all", status_code=status.HTTP_200_OK, summary="Get All Doctors", description="Retrieve a list of all doctors")
async def get_all_doctors():
    try:
        doctors = list(doctors_collection.find({"role": "doctor"}, {"_id": 0, "password": 0}))
...
@router.get("/specialization", status_code=status.HTTP_200_OK, summary="Get Doctors by Specialization",
             description="Retrieve a list of doctors by specialization")
async def get_doctors_by_specialization(specialization: str):
    try:
        doctors = list(doctors_collection.find({"role": "doctor", "specialization": specialization}, {"_id": 0, "password": 0}))
        if not doctors:
            raise HTTPException(
...
@router.put("/update/profile",status_code=status.HTTP_200_OK,summary="Update Doctor Profile")
...
@router.post("/add/appointment", status_code=status.HTTP_200_OK, summary="Add Appointment Slots")
async def add_appointment_slots(available_timings: list[AvailabilitySlot], current_user_role: dict = Depends(read_profile)):
    try:
        if current_user_role["role"] != "doctor":
            raise HTTPException(
                status_code=403,
                detail="Only doctors can add appointment slots"
            )
...
@router.delete("/remove/appointment",status_code=status.HTTP_200_OK,summary="Remove Appointment Slots")
async def remove_appointment_slots(slot_id: str,current_user_role: str = Depends(read_profile)):
    try:
        ...
        doctor = doctors_collection.find_one({"email": current_user_role["email"],"role": "doctor"})
...
@router.get("/appointments/all",status_code=status.HTTP_200_OK,summary="Get All Appointment Slots")
async def get_all_appointment_slots(doctor_email: str):
    try:
        doctor = doctors_collection.find_one({"email": doctor_email,"role": "doctor"})
...
@router.post("/add/patient_notes", status_code=status.HTTP_200_OK, summary="Add Notes to Patient Profile")
...
@router.put("/appointment/status",status_code=status.HTTP_200_OK, summary="Change appointment status")
async def change_status(appointment_status: str, slot_ID: str, current_user_role: dict = Depends(read_profile)):
    ...
    # Check appointment exists and belongs to doctor same doctor
    ...
    if result.modified_count == 0:
        logger.error("couldn't cange appointment status")
