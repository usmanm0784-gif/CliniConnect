...
from .patient_operation import patient_notes, fetch_patient
...
async def change_status_api(appointment_status, slot_ID, background_tasks, current_user):
    return await update_appointment_status(appointment_status, slot_ID, background_tasks, current_user)

async def get_patient_api(email, current_user_role):
    return await fetch_patient(email, current_user_role)
