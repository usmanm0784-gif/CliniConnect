...
async def add_slot(available_timings, current_user_role):
    try:
        ...
        for slot in available_timings:
            ...
            await create_slot(doctor_id, user["email"], slot.date, slot.start_time, slot.end_time, status= "available")
...
async def remove_slot(slot_id, current_user_role):
    try:
        ...
        return api_response(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            success=0,
            message=f"An error occurred while removing appointment slots: {e}"
        )
