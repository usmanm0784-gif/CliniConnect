...
async def change_status(appointment_status: str, slot_ID: str, background_tasks: BackgroundTasks, current_user: dict = Depends(read_profile),):
    return await change_status_api(appointment_status, slot_ID, background_tasks, current_user)

@router.get("/get/patient", summary="get patient")
async def get_patient(email: str, current_user: dict = Depends(read_profile)):
    return await get_patient_api(email, current_user)